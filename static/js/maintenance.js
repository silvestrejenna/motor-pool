...
function editMRow(id) {
    ...
    date.innerHTML =
        `<input type="date" id="m-date-${id}" value="${date.innerText.trim()}">`;

    vehicle.innerHTML =
        `<input type="text" id="m-vehicle-${id}" value="${vehicle.innerText.trim()}">`;

    problem.innerHTML =
        `<input type="text" id="m-problem-${id}" value="${problem.innerText.trim()}">`;

    action.innerHTML =
        `<input type="text" id="m-action_taken-${id}" value="${action.innerText.trim()}">`;

    const cleanCost = cost.innerText.replace('₱', '').trim();
    cost.innerHTML =
        `<input type="number" id="m-cost-${id}" value="${cleanCost}">`;

    mechanic.innerHTML =
        `<input type="text" id="m-mechanic-${id}" value="${mechanic.innerText.trim()}">`;
...
function editPMS(id) {
    ...
    vehicleCell.innerHTML =
        `<input type="text" id="p-vehicle-${id}" value="${vehicleCell.innerText.trim()}">`;

    lastCell.innerHTML =
        `<input type="date" id="p-last_pms_date-${id}" value="${lastDate || ''}">`;

    const kmValue = kmCell.innerText.replace('km', '').trim();
    kmCell.innerHTML =
        `<input type="number" id="p-km-${id}" value="${kmValue}">`;

    const oilValue = oilCell.innerText.replace('L', '').trim();
    oilCell.innerHTML =
        `<input type="number" step="0.01" id="p-oil_liters-${id}" value="${oilValue}">`;

    nextCell.innerHTML =
        `<input type="date" id="p-next_pms_date-${id}" value="${nextDate || ''}">`;
